...
// Función para crear un div nivel con materias
function crearNivel(codigo, nombre, materiasNivel) {
  const nivelDiv = document.createElement("div");
  nivelDiv.classList.add("nivel");
  nivelDiv.dataset.codigo = codigo;
  ...
  materiasNivel.forEach(materia => {
    ...
    materiaDiv.textContent = materia.nombre || materia.codigo;
    nivelDiv.appendChild(materiaDiv);
  });

  return nivelDiv;
}

// Función para crear el bloque electivas
function crearElectivas(electivas) {
  const electivasDiv = document.createElement("div");
  electivasDiv.classList.add("nivel", "electivas");

  const titulo = document.createElement("h3");
  titulo.textContent = "Electivas";
  electivasDiv.appendChild(titulo);

  electivas.forEach(electiva => {
    const electivaDiv = document.createElement("div");
    electivaDiv.classList.add("materia");
    electivaDiv.textContent = electiva.nombre;
    electivasDiv.appendChild(electivaDiv);
  });

  return electivasDiv;
}

// Aquí arma todo y lo mete en #malla-container
function armarMalla() {
  const contenedor = document.getElementById("malla-container");
  contenedor.innerHTML = ""; // limpio por si acaso

  // Agrupo materias por año (nivel)
  const niveles = {};
  materias.forEach(m => {
    if (!niveles[m.anio]) niveles[m.anio] = [];
    niveles[m.anio].push(m);
  });

  // Creo cada nivel (año) y lo agrego
  for (const anio in niveles) {
    const nivelDiv = crearNivel(anio, `Año ${anio}`, niveles[anio]);
    contenedor.appendChild(nivelDiv);
  }

  // Creo y agrego el bloque electivas
  const electivasDiv = crearElectivas(electivas);
  contenedor.appendChild(electivasDiv);
}
